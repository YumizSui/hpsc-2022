...
int main (int argc, char** argv) {
  ...
  hid_t file = H5Fopen("data2.h5", H5F_ACC_RDONLY, plist);
  ...
  hsize_t Nlocal[2] = {N[0]/dim[0]/2, N[1]/dim[1]/2};
  hsize_t Nsize[2] = {Nlocal[0]*count[0],Nlocal[1]*count[1]};
  hsize_t offset[2] = {mpirank / dim[0], mpirank % dim[0]};
  for(int i=0; i<2; i++) offset[i] *= Nlocal[i];
  hid_t localspace = H5Screate_simple(2, Nsize, NULL);
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, Nlocal);
  H5Pclose(plist);
  vector<int> buffer(Nsize[0]*Nsize[1]);
  ...
  int sum = 0;
  for (int i=0; i<Nsize[0]*Nsize[1]; i++)
    sum += buffer[i];
  printf("sum=%d\n",sum);
